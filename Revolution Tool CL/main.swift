...
for name in ["clefairy", "clefable", "cleffa", "togepi", "togetic", "snubbull", "granbull"] {
	let mon = pokemon(name).stats
	mon.type1 = .type(fairy.index)
	mon.save()
}

for name in ["marill", "azumarill", "clefairy", "clefable", "jigglypuff", "wigglytuff", "mr. mime", "cleffa", "igglybuff", "togepi", "snubbull", "granbull", "mawile", "ralts", "kirlia", "gardevoir"] {
	let mon = pokemon(name).stats
