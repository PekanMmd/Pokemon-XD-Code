...
class XGWarp : NSObject {
	...
	var file : XGFiles!
	var rawData : [Int] {
		return file.data.getByteStreamFromOffset(startOffset, length: kSizeOfWarp)
	}
	...
	init(file: XGFiles, index: Int, startOffset: Int) {
		...
		let w = data.getByteAtOffset(startOffset + kWarpTypeOffset)
	...
	func save() {
		...
		data.replaceByteAtOffset(startOffset + kWarpTypeOffset, withByte: self.warpType.index)
...
class XGMapRel : XGRelocationTable {
	...
	override init(file: XGFiles) {
		...
		for i in 0 ..< numberOfWarps {
			let warp = XGWarp(file: file, index: i, startOffset: firstWarp + (i * kSizeOfWarp))
			warps.append(warp)
		}
