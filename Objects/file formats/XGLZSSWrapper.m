...
- (UInt8 *)compressFile: (UInt8[])bytes ofSize:(int) size {
	...
	unsigned char *inFile = (unsigned char *)calloc(size, sizeof(unsigned char));
	...
	unsigned char *outFile = (unsigned char *)calloc(size, sizeof(unsigned char));
	
	_outLength = lzss_compress(inFile, size, outFile, size);
...
- (UInt8 *)decompressFile: (UInt8[])bytes ofSize:(int) inSize decompressedSize: (int) outSize {
	...
	unsigned char *inFile = (unsigned char *)calloc(inSize, sizeof(unsigned char));
	...
	unsigned char *outFile = (unsigned char *)calloc(outSize, sizeof(unsigned char));
	
	_outLength = unlzss(inFile, inSize, outFile, outSize);
